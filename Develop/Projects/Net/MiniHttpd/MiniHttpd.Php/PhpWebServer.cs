...
namespace MiniHttpd.Php
{
	...
	public class PhpWebServer : HttpWebServer
	{
		...
		/// <param name="port">An open port of range 1-65535 to listen on. Specify 0 to use any open port.</param>
		public PhpWebServer(IPAddress localAddress, int port) : this(localAddress, port, new VirtualDirectory("/", null))
		{
		}
		...
		/// <param name="port">An open port of range 1-65535 to listen on. Specify 0 to use any open port.</param>
		/// <param name="root">An <see cref="IDirectory"/> specifying the root directory. Its <c>Parent</c> must be <c>null</c>.</param>
		public PhpWebServer(IPAddress localAddress, int port, IDirectory root) : base(localAddress, port)
		{
			base.DefaultPages.Add("index.php");
			base.DefaultPages.Add("default.php");
		}
		...
		static bool IsPhpFile(string path)
		{
			string ext = Path.GetExtension(path);
			if(string.Compare(ext, ".php", true, System.Globalization.CultureInfo.InvariantCulture) == 0)
				return true;
			else
				return false;
		}
		...
		protected override void SendFileOrIndex(HttpRequest request, IResource resource)
		{
			for(IResource current = resource; current != null; current = current.Parent)
			{
				...
				{
					PhpAppDirectory app = current as PhpAppDirectory;
