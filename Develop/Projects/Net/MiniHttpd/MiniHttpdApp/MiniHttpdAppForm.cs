...
namespace MiniHttpdApp
{
	...
	public class MiniHttpdAppForm : Form
	{
		...
		class Settings
		{
			...
			class FolderSelector : UITypeEditor
			{
		...
		void LoadSettings()
		{
			...
			try
			{
				WindowState = (FormWindowState)Enum.Parse(typeof(FormWindowState), docElement["WindowState"].InnerText);
			}
			catch { }
		...
		private void server_StateChanged(object sender, EventArgs e)
		{
			if(InvokeRequired)
			{
				Invoke(new EventHandler(server_StateChanged), new[] {sender, e});
				return;
			}
