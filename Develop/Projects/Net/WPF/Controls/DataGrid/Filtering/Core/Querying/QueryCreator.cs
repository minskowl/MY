    ...
    public class QueryCreator
    {
        ...
        public void CreateFilter(ref Query query)
        {
            ...
            foreach (KeyValuePair<string, FilterData> kvp in filtersForColumns)
            {
                ...
                if (partialFilter.Length > 0) filter.Append(kvp.Value.ValuePropertyBindingPath + " != null AND ");//eliminate: Nullable object must have a value and object fererence not set to an object
            ...
            out object parameterValue, string stringValue, Type type)
        {
            ...
            try
            {
                ...
                {
                    parameterValue = Convert.ChangeType(stringValue, typeof(Decimal));//TODO use "real" number type
                }
