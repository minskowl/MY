...
using Savchin.CodeEditor.Services.File;
using Savchin.CodeEditor.Services.Parse.Doozer;
using Savchin.CodeEditor.Services.ParseService;
...
namespace Savchin.CodeEditor.Services.Parse
{
    /// <summary>
    ...
    public static class ParserService
    {
            ...
            get
            {
                lock (syncLock)
                {
                    if (defaultProjectContent == null)
                    {
                        ...
                        defaultProjectContent.AddReferencedContent(new ReflectionProjectContent(typeof(Window).Assembly, AssemblyParserService.DefaultProjectContentRegistry));
                    }
                    return defaultProjectContent;
                }
            }
        ...
        {
            return WorkbenchSingleton.SafeThreadFunction(GetParseableFileContentInternal, fileName);
        }

        private static List<SharpDevelopTextEditor> _editors = new List<SharpDevelopTextEditor>();
        ...
        static ITextBuffer GetParseableFileContentInternal(string fileName)
        {
            ...
            var editor = _editors.FirstOrDefault(e => e.FileName == fileName);
            if (editor != null)
                return new StringTextBuffer(editor.Text);
