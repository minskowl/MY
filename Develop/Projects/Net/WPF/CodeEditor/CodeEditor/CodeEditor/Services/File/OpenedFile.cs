...
namespace Savchin.CodeEditor.Services.File
{
	/// <summary>
	...
	public abstract class OpenedFile : ICanBeDirty
	{
		...
		public virtual void SaveToDisk()
		{
			...
			bool safeSaving = FileService.SaveUsingTemporaryFile && System.IO.File.Exists(FileName);
			...
			if (safeSaving) {
				DateTime creationTime = System.IO.File.GetCreationTimeUtc(FileName);
				System.IO.File.Delete(FileName);
				try {
					System.IO.File.Move(saveAs, FileName);
				...
				System.IO.File.SetCreationTimeUtc(FileName, creationTime);
			}
			IsDirty = false;
		}
...
//				SavedCurrentView(this, EventArgs.Empty);
		...
		void ReloadFromDiskInternal()
		{
			...
			if (currentView != null) {
				try {
					inLoadOperation = true;
                    ICSharpCode.Core.Properties memento = GetMemento(currentView);
					using (Stream sourceStream = OpenRead()) {
        ...
        static ICSharpCode.Core.Properties GetMemento(IViewContent viewContent)
		{
        ...
        static void RestoreMemento(IViewContent viewContent, ICSharpCode.Core.Properties memento)
		{
			if (memento != null) {
