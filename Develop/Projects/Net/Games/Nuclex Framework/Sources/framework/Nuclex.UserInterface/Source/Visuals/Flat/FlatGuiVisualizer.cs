...
namespace Nuclex.UserInterface.Visuals.Flat
{
    ...
    public class FlatGuiVisualizer : IGuiVisualizer, IDisposable
    {
        ...
        {
            using (var skinStream = new FileStream(skinPath, FileMode.Open,
                FileAccess.Read, FileShare.Read))
            {
                var contentManager = new ContentManager(serviceProvider, Path.GetDirectoryName(skinPath));
                try
                {
        ...
        public static FlatGuiVisualizer FromResource(IServiceProvider serviceProvider, ResourceManager resourceManager, string skinResource)
        {
            ...
            using (var skinStream = new MemoryStream(resourceData, false))
            {
                var manager = new ResourceContentManager(serviceProvider, resourceManager);
                return new FlatGuiVisualizer(manager, skinStream);
            }
        ...
        /// </param>
        public FlatGuiVisualizer(ContentManager contentManager, Stream skinStream)
            : this(new FlatGuiGraphics(contentManager, skinStream))
        {

        }
        /// <summary>
        /// Initializes a new instance of the <see cref="FlatGuiVisualizer"/> class.
        /// </summary>
        /// <param name="provider">The provider.</param>
        public FlatGuiVisualizer(IServiceProvider provider)
            : this(new FlatGuiGraphics(provider))
        {
        ...
        public void Draw(Screen screen)
        {
            ...
            try
            {
                ...
                while (controlStack.Count > 0)
                {
                    ...
                    if (!currentControl.IsVisible) continue;
        ...
        private void renderControl(Controls.Control controlToRender)
        {
            ...
            if (controlType == typeof(Controls.Control))
            {
                return;
            }
