...
namespace Savchin.Forms.Docking
{
	public class FloatWindow : Form, INestedPanesContainer, IDockDragSource
	{
		...
		internal const int WM_CHECKDISPOSE = (int)(WM.WM_USER + 1);
		...
		{
			if (m.Msg == (int)WM.WM_NCLBUTTONDOWN)
			{
				if (IsDisposed)
					return;

				uint result = NativeMethods.SendMessage(this.Handle, (int)WM.WM_NCHITTEST, 0, (uint)m.LParam);
            ...
            else if (m.Msg == (int)WM.WM_NCRBUTTONDOWN)
            {
                uint result = NativeMethods.SendMessage(this.Handle, (int)WM.WM_NCHITTEST, 0, (uint)m.LParam);
            ...
            else if (m.Msg == (int)WM.WM_NCLBUTTONDBLCLK)
            {
                uint result = NativeMethods.SendMessage(this.Handle, (int)WM.WM_NCHITTEST, 0, (uint)m.LParam);
                if (result != 2)	// HITTEST_CAPTION
                {
		...
		internal void TestDrop(IDockDragSource dragSource, DockOutlineBase dockOutline)
		{
            if (VisibleNestedPanes.Count == 1)
            {
                ...
                if (NativeMethods.SendMessage(Handle, (int)WM.WM_NCHITTEST, 0, lParam) == (uint)Win32.HitTest.HTCAPTION)
