...
function eXcell_ed(cell){
 ...
 this.val = this._htmlDecode(this.getValue());
 ...
 this.obj.onfocus = function () { this.select(); }
 this.obj.maxLength = this.grid.maxTextLenght;
...
this._isNewRowEditor = function()
{
    return this.grid.isNewRowEditor(
        this.grid.getRowId(
            this.cell.parentNode.rowIndex));
}

this._htmlEncode = function(value)
{
...
this._htmlDecode = function(value)
{
    value = value.replace(/&lt;/g, '<');
    value = value.replace(/&gt;/g, '>');
    value = value.replace(/&amp;/g, '&');
 ...
 this.setValue(this._htmlEncode(this.obj.value));
...
function eXcell_ch(cell){
 ...
 this.disabledF=function(fl){
    var checkIdAttr = '';
    if (this.getValue() == '1')
        checkIdAttr = "checked='true'";
    var disabledAttr = '';
    if((fl==true)||(fl==1))
        disabledAttr = "disabled='true'";
    this.cell.innerHTML = "<input id='" + this.grid.checkBoxItemPrefix + this.cell.id + "' name='" + this.grid.checkBoxItemPrefix + this.cell.id + "' type='checkbox' onclick='this.parentNode.obj.changeState()'" + checkIdAttr + disabledAttr + ">";        
}
 ...
 try{
  ...
  chObj = document.getElementById(this.grid.checkBoxItemPrefix + this.cell.id); 
  if (chObj)
  {
    if (chObj.checked)
        this.cell.chstate = '1';
...
// this.setCValue("<img src='"+this.grid.imgURL+"item_chk"+val+".gif' onclick='this.parentNode.obj.changeState()'>",this.cell.chstate);
 ...
 if (val == '1')
 this.setCValue("<input id='" + this.grid.checkBoxItemPrefix + this.cell.id + "' name='" + this.grid.checkBoxItemPrefix + this.cell.id + "' type='checkbox' onclick='this.parentNode.obj.changeState()' checked='true'>",this.cell.chstate);
else
 this.setCValue("<input id='" + this.grid.checkBoxItemPrefix + this.cell.id + "' name='" + this.grid.checkBoxItemPrefix + this.cell.id + "' type='checkbox' onclick='this.parentNode.obj.changeState()' >",this.cell.chstate);

    if (this.grid.onCheckBoxCreated)
        this.grid.onCheckBoxCreated(this);
}
