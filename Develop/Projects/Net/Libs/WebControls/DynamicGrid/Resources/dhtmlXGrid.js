...
/* Dragon */
    this._downMoving = false;
    this.objBoxCssClassName = 'objBox';

    this.checkBoxItemPrefix = 'test';
    this.onCheckBoxCreated = null;
    this.onRowAdded  = null;
    this.onAllRowsAdded = null;

    this.curColumnKey = null;
    this.isAscSort   = true;

    this.updateSort = function (columnKey)
    {
        if (this.curColumnKey == columnKey)
            this.isAscSort = !this.isAscSort;
        if (this.curColumnKey != columnKey)
            this.isAscSort = true;
        this.curColumnKey = columnKey;    
    }

    this.useActionTemplate = false;    
    this.actionTemplate = null;
    
    this.newRowPromptText = '';
    this.newRowIdPrefix = '';
    this.newRowUserDataName = 'IsNewRowEditor';
    this.maxTextLenght = 0;

    this._editorRowId = '';

    this._getNewRowId = function()
    {
        var newRowId = 0;
        while (-1 != this.getRowIndex(this.newRowIdPrefix + newRowId))
        {
            newRowId++;
        }
        return this.newRowIdPrefix + newRowId;
    }

    this.isNewRowEditor = function(rowId)
    {
       return this._editorRowId == rowId;
    }

    this.addNewRowEditor = function()
    {
        var newRowId = this._getNewRowId();
        this.addRowWithTemplate(newRowId, [this.newRowPromptText]);
        this._editorRowId = newRowId;
    }

    
    this.addRowWithTemplate = function(rowId, columnValues)
    {
        if (this.useActionTemplate)
        {
            var template = this.actionTemplate;
            if (template != '')
                template = template.replace('{0}', rowId);
            columnValues.push(template);
        }
        this.addRow(rowId, columnValues);
    }
    ...
    this.serializeData = function()
    {
        ...
        for (var rowIndex = 0; rowIndex < rowCount; rowIndex++)
        {
            ...
            if (this.isNewRowEditor(rowId))
                continue; // This is row of new item editor
            ...
            {
                xml += '<cell>';
                xml += this.cells(rowId, columnIndex).getValue();
                xml += '</cell>';
            }
   ...
   this.getQuestionsOrderedIdList = function()
   {
      var rowCount =  this.getRowsNum();
      ...
      {
          questionIndexes += this.getRowId(i) + ',';
      }

      if (questionIndexes.length > 0)
        questionIndexes = questionIndexes.substr(0, questionIndexes.length - 1);
...
/* end of dragon */
 ...
 this.objBox.className = this.objBoxCssClassName;
 ...
 if ((this._drsclmW) && (this.cellWidthPX[i] < this._drsclmW[i]))
 {
	this.cellWidthPX[i] =  this._drsclmW[i];		
 } 
}
}
 ...
 this.editStop = function(escPressed){
 if(_isOpera)
 if(this._Opera_stop){
 ...
 var g=this.editor;
 this.editor=null;
 var z=this.onEditCell(2,this.row.idd,this.cell._cellIndex,g.getValue(),g._htmlEncode(g.val), escPressed);
 ...
 if(ind<r.childNodes.length)
{
 ...
 var arLab = null;
 if (splitSign)
    arLab = hdrStr.split(splitSign);
 else   
    arLab = hdrStr.split(this.delim);
