...
namespace Savchin.Collection.Sorting
{
    ...
    /// </summary>
    internal struct SimpleTokenizer
    {
        ...
        public bool AdvanceIfIdent(String identity)
        {
            ...
            if (endPos <= expression.Length && (endPos == expression.Length || Char.IsLetterOrDigit(expression, endPos) == false))
            {
        ...
        public String ReadIdentity()
        {
            ...
            while(position < expression.Length && Char.IsLetterOrDigit(expression, position) ) { position++; }
