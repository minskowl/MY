...
namespace Savchin.Monads
{
    public static class MaybeNullable
    {
        ...
        /// </returns>
        public static Tuple<TSource?, Exception> TryDo<TSource>(this TSource? source, Action<Nullable<TSource>> action)
                where TSource : struct
        {
            if (source.HasValue != true) return new Tuple<TSource?, Exception>(source, null);
